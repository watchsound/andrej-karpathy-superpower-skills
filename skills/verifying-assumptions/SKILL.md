...
- **Re-anchor on intent.** Re-read the **currently-active** task as your human partner phrased it — not necessarily the session's first prompt. The signal that the session has pivoted is **semantic, not lexical**: ask whether the topic, module, file scope, or bounded context being discussed is materially different from the original task. Explicit pivot phrases (*"now let's", "switching to", "back to", "let's also check", "different angle"*) are common cues but not exhaustive — many pivots happen without an announcement phrase, and a phrase-list check has known recall gaps. The underlying check is the focus shift itself. When you detect one, re-anchor on the *current* frame's intent and ask: *"What higher-level goal does this assumption serve in the current frame?"* Verify against that goal, not the assumption text. The assumption itself may have been the wrong question — or it may have been asked under a stale frame.
